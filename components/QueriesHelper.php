...
class QueriesHelper
{
    ...
    public static function getResponseCountries()
    {
        $query = <<<'SQL'
        select
          distinct replace(json_extract(rs.response, '$."Country"'), '"', '') as "country"
        from respondent_survey rs where status <> :status
        and replace(json_extract(rs.response, '$."Country"'), '"', '') is not null
        order by 1
SQL;
        $query = <<<SQL
        select
          distinct rs.response as "country"
        from respondent_survey rs where status <> :status
        and rs.response like '%"Country"%'
        order by 1
SQL;
        ...
        $list = ArrayHelper::getColumn($list, 'country');

        foreach($list as $k => $v) {
            $json = json_decode($v);
            $list[$k] = isset($json->Country) ? FormatHelper::clearResponse($json->Country) : null;
        }
    ...
    {
        $list = \Yii::$app->db->createCommand('select id, name from country order by 2')->queryAll();

        $listNames = ArrayHelper::getColumn($list, 'name');
        $listIds = ArrayHelper::getColumn($list, 'id');

        return array_combine($listIds, $listNames);
    }
